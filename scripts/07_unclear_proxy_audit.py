...
from threat_model_analysis_v2_lib import DEFAULT_LABELS_INPUT, DEFAULT_PAIR_DATASET, DEFAULT_TOPIC_INPUT, V2Config, prepare_eligible_outputs, run_unclear_proxy_audit
...
def main() -> int:
    ...
    eligible_path = config.output_dir / "data" / "eligible_question_method_rows.csv"
    eligible = pd.read_csv(eligible_path) if eligible_path.exists() else prepare_eligible_outputs(config)
    run_unclear_proxy_audit(config, eligible)
    print(config.output_dir / "audit" / "unclear_proxy_examples.md")
    return 0
