...
Hypothesis: both subsets preserve mid-layer knowledge, but suppressed shows
stronger late-layer and output-access collapse.

Per-question metrics computed for retained and suppressed:
  - mid-layer trajectory AUC  (layers 8-24, mean across ck1-ck8)
  - late-layer trajectory AUC (layers 25-32, mean across ck1-ck8)
  - mid K at ck8
  - late K at ck8
  - mid-minus-late gap at ck8
  - external K at base
  - external K at ck8
  - external drop  (K_ext_base - K_ext_ck8)
  - internal/external gap at ck8  (K_int_mid_ck8 - K_ext_ck8)

Outputs (plots/retained_vs_suppressed_mechanism/):
  retained_vs_suppressed_mechanism.csv
  retained_vs_suppressed_mechanism.md
  retained_vs_suppressed_per_method.pdf / .png   (per-method 4-panel boxes)
  retained_vs_suppressed_summary_bars.pdf / .png  (cross-method summary)
"""
import sys
import matplotlib
matplotlib.use("Agg")
...
from hk_utils import (
    ...
    compute_prefilter_mask, get_parquet_multi_single, get_cv_layer_df,
    get_method_ck8_labels, method_fname,
)
...
COL_RET  = "#1f77b4"   # blue  — retained
COL_SUPP = "#d62728"   # red   — suppressed
SUBSETS  = ["retained", "suppressed"]

METRICS = {
    "mid_auc":        "Mid-layer traj. AUC\n(layers 8-24, ck1-ck8)",
    "late_auc":       "Late-layer traj. AUC\n(layers 25-32, ck1-ck8)",
    "ext_drop":       "External drop\n(K_ext base - K_ext ck8)",
    "int_ext_gap_ck8":"Int-ext gap @ ck8\n(K_int_mid_ck8 - K_ext_ck8)",
}
...
def load_per_question_table():
    ...
    # pivot so each question has one row with mid/late columns
    mid  = traj[traj["layer_band"] == "mid" ][["method","subset","question_idx","auc","ck8"]].copy()
    late = traj[traj["layer_band"] == "late"][["method","subset","question_idx","auc","ck8"]].copy()
    mid  = mid.rename(columns={"auc": "mid_auc",  "ck8": "mid_ck8"})
    late = late.rename(columns={"auc": "late_auc", "ck8": "late_ck8"})

    merged = mid.merge(late, on=["method", "subset", "question_idx"], how="inner")
    merged["mid_minus_late_ck8"] = merged["mid_ck8"] - merged["late_ck8"]
    return merged


def load_external_k():
    """Return DataFrame with (method, question_idx, k_ext_base, k_ext_ck8)."""
    ...
    # Base external K
    base_ext = parquet_multi[parquet_multi["model_id"] == "base"][["question_idx","k_external"]]
    base_ext = base_ext.rename(columns={"k_external": "k_ext_base"})

    rows = []
    for method in METHODS:
        labels, k_ext_ck8_arr, _ = get_method_ck8_labels(
            method, correct_idx, filt_te, filt_o2p, parquet_multi
        )
        if labels is None:
            continue
...
def compute_stats(per_q):
    ...
    for method in METHODS:
        ...
        for metric, label in METRICS.items():
            ...
            # direction: retained > suppressed for mid_auc; suppressed > retained for ext_drop/gap
...
def plot_summary_bars(stats_df):
    """Cross-method grouped bars for each metric: mean suppressed - mean retained."""
    metric_keys  = list(METRICS.keys())
    metric_labels_short = ["Mid traj AUC", "Late traj AUC", "Ext drop", "Int-ext gap ck8"]
...
def write_md(stats_df):
    lines = [
        ...
        "",
        "Hypothesis: both subsets preserve mid-layer internal knowledge, but suppressed "
        "shows stronger late-layer and output-access collapse.",
        "",
        "p-values: one-sided Mann-Whitney U (direction: suppressed > retained for "
        "ext_drop and int_ext_gap; retained > suppressed for mid/late AUC).",
        "",
    ]
    for metric, label in METRICS.items():
