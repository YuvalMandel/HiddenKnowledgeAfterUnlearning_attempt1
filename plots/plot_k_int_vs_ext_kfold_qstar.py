...
Q* = questions where base model has k_internal=1 AND k_external=1 (in their
single test fold). Each question appears in exactly one test fold, so the
Q* mask is derived directly from the base CV parquet.
...
print(f"Q* size: {len(qstar)} questions (base k_int=k_ext=1)")
...
fig.tight_layout()
out = os.path.join(os.path.dirname(__file__), "k_int_vs_ext_kfold_qstar")
