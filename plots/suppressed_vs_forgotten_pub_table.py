...
def cohens_d(a, b):
    """Pooled-SD Cohen's d (suppressed minus forgotten)."""
    na, nb = len(a), len(b)
...
def cliffs_delta(a, b):
    """Cliff's delta: proportion of (a>b) pairs minus (a<b) pairs, range [-1,1]."""
...
def main():
    ...
    mid = df[(df["layer_band"] == "mid") & (df["subset"].isin(["suppressed", "forgotten"]))].copy()

    rows = []
    for method in METHOD_ORDER:
        m = mid[mid["method"] == method]
    ...
    lines = [
        ...
        "",
        "Mid-layer internal trajectory AUC. Bootstrap 95% CI (n=5000). "
