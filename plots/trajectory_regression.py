#!/usr/bin/env python3
"""
Task G: Reframe prediction as continuous regression on trajectory targets.

Base features (8) -> predict continuous trajectory outcomes per question×method:
  1. mid_traj_auc       — mean K_int across ck1-ck8, layers 8-24
  2. late_traj_auc      — mean K_int across ck1-ck8, layers 25-32
  3. mid_ck8            — K_int at ck8, layers 8-24
  4. ext_drop           — K_ext_base - K_ext_ck8
  5. int_ext_gap_ck8    — K_int_mid_ck8 - K_ext_ck8
...
from hk_utils import (
    ...
    compute_prefilter_mask, get_parquet_multi_single, get_method_ck8_labels,
    method_fname,
)
...
TARGETS = {
    "mid_traj_auc":    "Mid-layer traj. AUC (ck1-ck8)",
    "late_traj_auc":   "Late-layer traj. AUC (ck1-ck8)",
    "mid_ck8":         "Mid-layer K_int @ ck8",
...
def load_targets():
    ...
    mid  = traj[traj["layer_band"] == "mid"][["method","question_idx","auc","ck8"]].rename(
        columns={"auc": "mid_traj_auc", "ck8": "mid_ck8"})
    late = traj[traj["layer_band"] == "late"][["method","question_idx","auc"]].rename(
        columns={"auc": "late_traj_auc"})
    targets = mid.merge(late, on=["method","question_idx"], how="inner")
    ...
    base_ext = parquet_multi[parquet_multi["model_id"] == "base"][["question_idx","k_external"]].rename(
    ...
    targets = targets.merge(ext[["method","question_idx","ext_drop","k_ext_ck8"]],
                            on=["method","question_idx"], how="left")
    targets["int_ext_gap_ck8"] = targets["mid_ck8"] - targets["k_ext_ck8"]
    return targets
...
def run_cv_regression(X, y, model_name, model):
    ...
    # cross_val_predict requires non-repeating partitions
...
def get_feature_importances(X, y, model_name, model):
    ...
    if hasattr(m, "coef_"):
        ...
        coef = m.coef_ * sc.scale_  # unstandardize for comparability
        return np.abs(coef)
...
def plot_feature_importance(importances_dict, target_key, target_label):
    # average importances across models that have them
    arr = np.array(list(importances_dict.values()))
    # normalize each model's importances to sum to 1 before averaging
...
def main():
    ...
    lines = [
        ...
        "",
        "Base features (8) predicting continuous trajectory targets.",
