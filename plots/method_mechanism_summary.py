...
Assembles per-method metrics from Tasks C, D, E and the binary classifiers:
  ...
  - suppressed-minus-forgotten mid-AUC diff + effect size + sig (Task C)
  - suppressed-minus-forgotten late-AUC diff (new)
...
def compute_late_sf_diff(traj):
    """Suppressed-minus-forgotten late-layer trajectory AUC diff per method."""
    late = traj[(traj["layer_band"] == "late") &
                (traj["subset"].isin(["suppressed", "forgotten"]))].copy()
    rows = []
    for m in METHOD_ORDER:
        s = late[(late["method"] == m) & (late["subset"] == "suppressed")]["auc"].dropna().values
        f = late[(late["method"] == m) & (late["subset"] == "forgotten")]["auc"].dropna().values
        if len(s) < 2 or len(f) < 2:
            rows.append({"method": m, "late_auc_diff_sf": np.nan, "late_sig": ""})
            continue
        diff = s.mean() - f.mean()
        _, p = mannwhitneyu(s, f, alternative="greater")
        sig = "***" if p < 0.001 else ("**" if p < 0.01 else ("*" if p < 0.05 else "ns"))
        rows.append({"method": m, "late_auc_diff_sf": round(diff, 4), "late_sig": sig})
...
def build_summary():
    ...
    # ── counts ──────────────────────────────────────────────────────────────
    counts = (
        traj[(traj["layer_band"] == "mid") & (traj["subset"].isin(["retained","suppressed","forgotten","lucky"]))]
    ...
    # ── Task C: suppressed-forgotten mid-AUC ────────────────────────────────
    sf_mid = pub[["method","diff","cohens_d","cliffs_delta","sig","q_value"]].rename(columns={
        "diff":         "mid_auc_diff_sf",
    ...
    # ── late-AUC suppressed-forgotten ───────────────────────────────────────
    sf_late = compute_late_sf_diff(traj)

    # ── Task E: ext_drop retained-suppressed ────────────────────────────────
    ...
    for right in [sf_mid, sf_late, ext_drop, sf_auc, sr_auc]:
...
def write_md(df):
    ...
    lines += [
        "",
        "## Suppressed vs Forgotten trajectory gap (mid and late layers)",
        "",
        "| Method | Mid-AUC diff | Cohen d | Cliff d | mid sig | Late-AUC diff | late sig |",
        "|---|---:|---:|---:|---:|---:|---:|",
    ]
    for _, r in df.iterrows():
        lines.append(
            f"| {r['method']} | {r['mid_auc_diff_sf']:.4f} | {r['cohens_d_sf']:.3f} | "
            f"{r['cliffs_delta_sf']:.3f} | {r['mid_sig']} | "
            f"{r['late_auc_diff_sf']:.4f} | {r['late_sig']} |"
        )
...
def plot_ranking(df):
    """
    4-panel horizontal bar chart, one panel per key metric.
    Methods sorted by prop_suppressed (hidden-knowledge mass) descending.
    """
    ...
    panels = [
        ...
        ("mid_auc_diff_sf",    "Supp-Forg mid-AUC diff",         "#1f77b4"),
    ...
    for ax, (col, title, color) in zip(axes, panels):
        ...
        bars = ax.barh(y, vals, color=color, alpha=0.75, edgecolor="white")

        # Reference line for classifier AUC panel
        if col == "clf_auc_sf":
            ax.axvline(0.5, color="black", lw=0.8, ls="--", zorder=0)

        # Significance stars for trajectory/ext_drop panels
        sig_col = {"mid_auc_diff_sf": "mid_sig",
...
def main():
    ...
    print(df[["method","suppressed","forgotten","prop_suppressed",
              "mid_auc_diff_sf","late_auc_diff_sf","ext_drop_diff_sr",
